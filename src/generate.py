...
# before running this task. Moreover, the trivial option can synthesize at most 
# $n$ images per label, where $n$ is the total number of images in the input 
# CAPS.
# ```
# ### Running the task
#
...
# This command needs a BIDS folder as argument in order to create the
# `missing_mods_directory` and the `merged.tsv` file, but when you already have
# these, you can give an empty folder as argument and provide the paths to the 
# required files separately as options.
