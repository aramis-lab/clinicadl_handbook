...
# The objective of the `reconstruction` is to learn to reconstruct images given in input. To do so, we used autoencoder.
...
# Here, as you will use slice-level, you simply need to type the following command line:
...
# In short, there is a folder for each feature (**image, slice, roi or patch**) and
...
# The package `clinicadl` is able to extract patches _on-the-fly_ (from
...
# ROI are 3D patches defined by masks that need to be at the root in CAPS_DIRECTORY. 
# In the data_adni/masks folder you can find masks of the right and left hippocampus that can be used to define two region of interest.
...
# - **single-network**: one network is trained on all patch locations / all regions.
...
# framework), however the network may be more accurate as they are specialized for
...
# The default network used for reconstruction in ClinicaDL is an autoencoder derived from the convolutional part of CNN with 5 convolutionnal layers followed by 3 fully connected.
# The compatible criterion loss are :
# - the mean squared error between the input and the network output ([link](https://pytorch.org/docs/stable/generated/torch.nn.MSELoss.html)),
# - the Kullback-Leibler divergence loss ([link](https://pytorch.org/docs/stable/generated/torch.nn.KLDivLoss.html)),
# - the mean absolute error loss ([link](https://pytorch.org/docs/stable/generated/torch.nn.L1Loss.html)), 
# - a combination of a Sigmoid layer and the BCELoss ([link](https://pytorch.org/docs/stable/generated/torch.nn.BCEWithLogitsLoss.html)), 
# - the Huber loss ([link](https://pytorch.org/docs/stable/generated/torch.nn.HuberLoss.html)),
# - the smooth L1 loss ([link](https://pytorch.org/docs/stable/generated/torch.nn.SmoothL1Loss.html))
...
#%% [markdown]
# ### Prerequisites
# You need to execute the `clinicadl tsvtool get-labels` and `clinicadl tsvtools {split|kfold}`commands
...
# For the reconstruction task, you can save the output tensors of a whole data group, associated with the tensor corresponding to their input. 
# This can be useful if you want to perform extra analyses directly on the images reconstructed by a trained network, 
# or simply visualize them for a qualitative check.
...
# If you want to add optionnal argument you can check the [documentation](https://clinicadl.readthedocs.io/en/latest/Predict/).
