...
# The cross-entropy loss between the ground truth and the network output is used to quantify the error made by the network during the training process, 
...
# The default CNN used for classification in `clinicadl` is `Conv5_FC3` which is a convolutional neural network with 5 convolution and 
# 3 fully-connected layer but in this notebook we will use the resnet18: 
...
# %% [markdown]
# The default label for the classification task is `diagnosis` but as long as it is a categorical variable; it can be of any type.
# The next cell train a resnet18 to classify 2D slices of t1-linear MRI by diagnosis (AD or CN).
